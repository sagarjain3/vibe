...
function NotificationCard({ noti }) {
    return (
        <div className='w-full min-h-[50px] bg-gray-800 rounded-full flex justify-between items-center  p-[5px]'>
            ...
            <div className='w-[40px] h-[40px] rounded-full overflow-hidden border-4 border-black'>
                ...
                {noti.loop ? <video src={noti?.loop?.media} muted className='h-full object-cover' /> : noti.post && noti?.post?.mediaType == "image" ? <img src={noti?.post?.media} className='h-full object-cover' /> : noti?.post ? <video src={noti?.post?.media} muted loop className='h-full object-cover' /> : null}
